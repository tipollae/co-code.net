...
var ViewPlugin = class _ViewPlugin {
  ...
  */
  static define(create, spec) {
    ...
    return new _ViewPlugin(nextPluginID++, create, eventHandlers, eventObservers, (plugin) => {
      let ext = [];
      if (deco)
        ext.push(decorations.of((view3) => {
          let pluginInst = view3.plugin(plugin);
  ...
  */
  static fromClass(cls, spec) {
    return _ViewPlugin.define((view3, arg) => new cls(view3, arg), spec);
  }
};
var PluginInstance = class {
  ...
  update(view3) {
    if (!this.value) {
      if (this.spec) {
        try {
          this.value = this.spec.plugin.create(view3, this.spec.arg);
        } catch (e) {
          logException(view3.state, e, "CodeMirror plugin crashed");
          this.deactivate();
        }
      }
...
function getIsolatedRanges(view3, line) {
  let isolates = view3.state.facet(bidiIsolatedRanges);
  if (!isolates.length)
    return isolates;
  let sets = isolates.map((i2) => i2 instanceof Function ? i2(view3) : i2);
  let result = [];
...
function getScrollMargins(view3) {
  let left = 0, right = 0, top2 = 0, bottom = 0;
  for (let source of view3.state.facet(scrollMargins)) {
    let m = source(view3);
    if (m) {
      if (m.left != null)
...
var ViewUpdate = class _ViewUpdate {
  constructor(view3, state3, transactions) {
    this.view = view3;
    this.state = state3;
    this.transactions = transactions;
    this.flags = 0;
    this.startState = view3.state;
  ...
  @internal
  */
  static create(view3, state3, transactions) {
    return new _ViewUpdate(view3, state3, transactions);
  }
  /**
...
var WidgetTile = class _WidgetTile extends Tile {
  ...
  static of(widget, view3, length, flags, dom) {
    if (!dom) {
      dom = widget.toDOM(view3);
      if (!widget.editable)
...
var TileUpdate = class {
  constructor(view3, old, blockWrappers2, decorations2, disallowBlockEffectsFor) {
    this.view = view3;
    this.decorations = decorations2;
    this.disallowBlockEffectsFor = disallowBlockEffectsFor;
    this.openWidget = false;
    this.openMarks = 0;
    this.cache = new TileCache(view3);
    this.text = new TextStream(view3.state.doc);
    this.builder = new TileBuilder(this.cache, new DocTile(view3, view3.contentDOM), RangeSet.iter(blockWrappers2));
    this.cache.reused.set(
      old,
      2
      /* Reused.DOM */
...
var DocView = class {
  constructor(view3) {
    ...
    this.updateDeco();
    this.tile = new DocTile(view3, view3.contentDOM);
    this.updateInner([new ChangedRange(0, 0, 0, view3.state.doc.length)], null);
  }
  ...
  enforceCursorAssoc() {
    if (this.hasComposition)
      return;
    let { view: view3 } = this, cursor = view3.state.selection.main;
    let sel = getSelection(view3.root);
    let { anchorNode, anchorOffset } = view3.observer.selectionRange;
    ...
    view3.observer.readSelectionRange();
    let newRange = view3.observer.selectionRange;
    if (view3.docView.posFromDOM(newRange.anchorNode, newRange.anchorOffset) != cursor.from)
...
function findCompositionNode(view3, headPos) {
  let sel = view3.observer.selectionRange;
  if (!sel.focusNode)
    return null;
  ...
  if (textAfter && textBefore && textAfter.node != textBefore.node) {
    ...
    if (!tileAfter || tileAfter.isText() && tileAfter.text != textAfter.node.nodeValue) {
      textNode = textAfter;
    } else if (view3.docView.lastCompositionAfterCursor) {
      let tileBefore = Tile.get(textBefore.node);
      if (!(!tileBefore || tileBefore.isText() && tileBefore.text != textBefore.node.nodeValue))
        textNode = textAfter;
    }
  }
  view3.docView.lastCompositionAfterCursor = textNode != textBefore;
  if (!textNode)
    return null;
  let from = headPos - textNode.offset;
  return { from, to: from + textNode.node.nodeValue.length, node: textNode.node };
}
function findCompositionRange(view3, changes, headPos) {
  let found = findCompositionNode(view3, headPos);
  if (!found)
    return null;
  let { node: textNode, from, to } = found, text = textNode.nodeValue;
  if (/[\n\r]/.test(text))
    return null;
  if (view3.state.doc.sliceString(found.from, found.to) != text)
    return null;
...
function posAtCoordsImprecise(view3, contentRect, block, x, y) {
  let into = Math.round((x - contentRect.left) * view3.defaultCharacterWidth);
  if (view3.lineWrapping && block.height > view3.defaultLineHeight * 1.5) {
    let textHeight = view3.viewState.heightOracle.textHeight;
    let line = Math.floor((y - block.top - (view3.defaultLineHeight - textHeight) * 0.5) / textHeight);
    into += line * view3.viewState.heightOracle.lineLength;
  }
  let content2 = view3.state.sliceDoc(block.from, block.to);
  return block.from + findColumn(content2, into, view3.state.tabSize);
}
function blockAt(view3, pos, side) {
  let line = view3.lineBlockAt(pos);
...
function moveToLineBoundary(view3, start, forward, includeWrap) {
  let line = blockAt(view3, start.head, start.assoc || -1);
  let coords = !includeWrap || line.type != BlockType.Text || !(view3.lineWrapping || line.widgetLineBreaks) ? null : view3.coordsAtPos(start.assoc < 0 && start.head > line.from ? start.head - 1 : start.head);
  if (coords) {
    let editorRect = view3.dom.getBoundingClientRect();
    let direction = view3.textDirectionAt(line.from);
    let pos = view3.posAtCoords({
...
function moveByChar(view3, start, forward, by) {
  let line = view3.state.doc.lineAt(start.head), spans = view3.bidiSpans(line);
  let direction = view3.textDirectionAt(line.from);
  for (let cur = start, check = null; ; ) {
    let next = moveVisually(line, spans, direction, cur, forward), char = movedOver;
    if (!next) {
      if (line.number == (forward ? view3.state.doc.lines : 1))
        return cur;
      char = "\n";
      line = view3.state.doc.line(line.number + (forward ? 1 : -1));
      spans = view3.bidiSpans(line);
      next = view3.visualLineSide(line, !forward);
    }
    if (!check) {
      if (!by)
        return next;
...
function byGroup(view3, pos, start) {
  let categorize = view3.state.charCategorizer(pos);
...
function moveVertically(view3, start, forward, distance) {
  let startPos = start.head, dir = forward ? 1 : -1;
  if (startPos == (forward ? view3.state.doc.length : 0))
    return EditorSelection.cursor(startPos, start.assoc);
  let goal = start.goalColumn, startY;
  let rect = view3.contentDOM.getBoundingClientRect();
  let startCoords = view3.coordsAtPos(startPos, start.assoc || ((start.empty ? forward : start.head == start.from) ? 1 : -1));
  let docTop = view3.documentTop;
  if (startCoords) {
    if (goal == null)
      goal = startCoords.left - rect.left;
    startY = dir < 0 ? startCoords.top : startCoords.bottom;
  } else {
    let line = view3.viewState.lineBlockAt(startPos);
    if (goal == null)
      goal = Math.min(rect.right - rect.left, view3.defaultCharacterWidth * (startPos - line.from));
    startY = (dir < 0 ? line.top : line.bottom) + docTop;
  }
  let resolvedGoal = rect.left + goal;
  let halfText = view3.viewState.heightOracle.textHeight >> 1, dist2 = distance !== null && distance !== void 0 ? distance : halfText;
  for (let scan = 0; ; scan += halfText) {
    let y = startY + (dist2 + scan) * dir;
    let pos = posAtCoords(view3, { x: resolvedGoal, y }, false, dir);
    if (forward ? y > rect.bottom : y < rect.top)
      return EditorSelection.cursor(pos.pos, pos.assoc);
    let posCoords = view3.coordsAtPos(pos.pos, pos.assoc), mid = posCoords ? (posCoords.top + posCoords.bottom) / 2 : 0;
...
function skipAtoms(view3, oldPos, pos) {
  let newPos = skipAtomicRanges(view3.state.facet(atomicRanges).map((f) => f(view3)), pos.from, oldPos.head > pos.from ? -1 : 1);
...
function posAtCoords(view3, coords, precise, scanY) {
  let content2 = view3.contentDOM.getBoundingClientRect(), docTop = content2.top + view3.viewState.paddingTop;
  let { x, y } = coords, yOffset = y - docTop, block;
  for (; ; ) {
    if (yOffset < 0)
      return new PosAssoc(0, 1);
    if (yOffset > view3.viewState.docHeight)
      return new PosAssoc(view3.state.doc.length, -1);
    block = view3.elementAtHeight(yOffset);
    if (scanY == null)
      break;
    if (block.type == BlockType.Text) {
      if (scanY < 0 ? block.to < view3.viewport.from : block.from > view3.viewport.to)
        break;
      let rect = view3.docView.coordsAt(scanY < 0 ? block.from : block.to, scanY > 0 ? -1 : 1);
      if (rect && (scanY < 0 ? rect.top <= yOffset + docTop : rect.bottom >= yOffset + docTop))
        break;
    }
    let halfLine = view3.viewState.heightOracle.textHeight / 2;
    yOffset = scanY > 0 ? block.bottom + halfLine : block.top - halfLine;
  }
  if (view3.viewport.from >= block.to || view3.viewport.to <= block.from) {
    if (precise)
      return null;
    if (block.type == BlockType.Text) {
      let pos = posAtCoordsImprecise(view3, content2, block, x, y);
  ...
  let line = view3.docView.lineAt(block.from, 2);
  if (!line || line.length != block.length)
    line = view3.docView.lineAt(block.from, -2);
  return new InlineCoordsScan(view3, x, y, view3.textDirectionAt(block.from)).scanTile(line, block.from);
}
var InlineCoordsScan = class {
  constructor(view3, x, y, baseDir) {
    this.view = view3;
    this.x = x;
    this.y = y;
...
var DOMReader = class {
  constructor(points, view3) {
    this.points = points;
    this.view = view3;
    this.text = "";
    this.lineSeparator = view3.state.facet(EditorState.lineSeparator);
  }
  append(text) {
    this.text += text;
  }
...
function isEmptyToEnd(node, end) {
  ...
  for (; ; node = node.nextSibling) {
    if (node == end || !node)
      break;
    let view3 = Tile.get(node);
    if (!(view3 === null || view3 === void 0 ? void 0 : view3.isWidget()))
      return false;
    if (view3)
      (widgets || (widgets = [])).push(view3);
  }
  if (widgets)
...
var DOMChange = class {
  constructor(view3, start, end, typeOver) {
    this.typeOver = typeOver;
    this.bounds = null;
    this.text = "";
    this.domChanged = start > -1;
    let { impreciseHead: iHead, impreciseAnchor: iAnchor } = view3.docView, curSel = view3.state.selection;
    if (view3.state.readOnly && start > -1) {
      this.newSel = null;
    } else if (start > -1 && (this.bounds = domBoundsAround(view3.docView.tile, start, end, 0))) {
      let selPoints = iHead || iAnchor ? [] : selectionPoints(view3);
      let reader = new DOMReader(selPoints, view3);
      reader.readRange(this.bounds.startDOM, this.bounds.endDOM);
      this.text = reader.text;
      this.newSel = selectionFromPoints(selPoints, this.bounds.from);
    } else {
      let domSel = view3.observer.selectionRange;
      let head = iHead && iHead.node == domSel.focusNode && iHead.offset == domSel.focusOffset || !contains(view3.contentDOM, domSel.focusNode) ? curSel.main.head : view3.docView.posFromDOM(domSel.focusNode, domSel.focusOffset);
      let anchor = iAnchor && iAnchor.node == domSel.anchorNode && iAnchor.offset == domSel.anchorOffset || !contains(view3.contentDOM, domSel.anchorNode) ? curSel.main.anchor : view3.docView.posFromDOM(domSel.anchorNode, domSel.anchorOffset);
      let vp = view3.viewport;
      if ((browser.ios || browser.chrome) && head != anchor && Math.min(head, anchor) <= curSel.main.from && Math.max(head, anchor) >= curSel.main.to && (vp.from > 0 || vp.to < view3.state.doc.length)) {
        let from = Math.min(head, anchor), to = Math.max(head, anchor);
        let offFrom = vp.from - from, offTo = vp.to - to;
        if ((offFrom == 0 || offFrom == 1 || from == 0) && (offTo == 0 || offTo == -1 || to == view3.state.doc.length)) {
          head = 0;
          anchor = view3.state.doc.length;
        }
      }
      if (view3.inputState.composing > -1 && curSel.ranges.length > 1) {
        this.newSel = curSel.replaceRange(EditorSelection.range(anchor, head));
      } else if (view3.lineWrapping && anchor == head && !(curSel.main.empty && curSel.main.head == head) && view3.inputState.lastTouchTime > Date.now() - 100) {
        let before = view3.coordsAtPos(head, -1), assoc = 0;
        if (before)
          assoc = view3.inputState.lastTouchY <= before.bottom ? -1 : 1;
...
function applyDOMChange(view3, domChange) {
  let change;
  let { newSel } = domChange, { state: state3 } = view3, sel = state3.selection.main;
  let lastKey = view3.inputState.lastKeyTime > Date.now() - 100 ? view3.inputState.lastKeyCode : -1;
  if (domChange.bounds) {
    ...
    let cmp = state3.doc.sliceString(from, to, LineBreakPlaceholder), selEnd, diff;
  ...
  } else if (newSel && (!view3.hasFocus && state3.facet(editable) || sameSelPos(newSel, sel))) {
    newSel = null;
  }
  if (!change && !newSel)
    return false;
  if ((browser.mac || browser.android) && change && change.from == change.to && change.from == sel.head - 1 && /^\. ?$/.test(change.insert.toString()) && view3.contentDOM.getAttribute("autocorrect") == "off") {
    if (newSel && change.insert.length == 2)
      newSel = EditorSelection.single(newSel.main.anchor - 1, newSel.main.head - 1);
    change = { from: change.from, to: change.to, insert: Text.of([change.insert.toString().replace(".", " ")]) };
  } else if (state3.doc.lineAt(sel.from).to < sel.to && view3.docView.lineHasWidget(sel.to) && view3.inputState.insertingTextAt > Date.now() - 50) {
    change = {
      from: sel.from,
      to: sel.to,
      insert: state3.toText(view3.inputState.insertingText)
    };
  ...
  if (change) {
    return applyDOMChangeInner(view3, change, newSel, lastKey);
  } else if (newSel && !sameSelPos(newSel, sel)) {
    let scrollIntoView2 = false, userEvent = "select";
    if (view3.inputState.lastSelectionTime > Date.now() - 50) {
      if (view3.inputState.lastSelectionOrigin == "select")
        scrollIntoView2 = true;
      userEvent = view3.inputState.lastSelectionOrigin;
      if (userEvent == "select.pointer")
        newSel = skipAtomsForSelection(state3.facet(atomicRanges).map((f) => f(view3)), newSel);
    }
    view3.dispatch({ selection: newSel, scrollIntoView: scrollIntoView2, userEvent });
    return true;
  } else {
    return false;
  }
}
function applyDOMChangeInner(view3, change, newSel, lastKey = -1) {
  if (browser.ios && view3.inputState.flushIOSKey(change))
    return true;
  let sel = view3.state.selection.main;
  if (browser.android && (change.to == sel.to && // GBoard will sometimes remove a space it just inserted
  // after a completion when you press enter
  (change.from == sel.from || change.from == sel.from - 1 && view3.state.sliceDoc(change.from, sel.from) == " ") && change.insert.length == 1 && change.insert.lines == 2 && dispatchKey(view3.contentDOM, "Enter", 13) || (change.from == sel.from - 1 && change.to == sel.to && change.insert.length == 0 || lastKey == 8 && change.insert.length < change.to - change.from && change.to > sel.head) && dispatchKey(view3.contentDOM, "Backspace", 8) || change.from == sel.from && change.to == sel.to + 1 && change.insert.length == 0 && dispatchKey(view3.contentDOM, "Delete", 46)))
    return true;
  let text = change.insert.toString();
  if (view3.inputState.composing >= 0)
    view3.inputState.composing++;
  let defaultTr;
  let defaultInsert = () => defaultTr || (defaultTr = applyDefaultInsert(view3, change, newSel));
  if (!view3.state.facet(inputHandler).some((h) => h(view3, change.from, change.to, text, defaultInsert)))
    view3.dispatch(defaultInsert());
  return true;
}
function applyDefaultInsert(view3, change, newSel) {
  let tr, startState = view3.state, sel = startState.selection.main, inAtomic = -1;
  if (change.from == change.to && change.from < sel.from || change.from > sel.to) {
    let side = change.from < sel.from ? -1 : 1, pos = side < 0 ? sel.from : sel.to;
    let moved = skipAtomicRanges(startState.facet(atomicRanges).map((f) => f(view3)), pos, side);
  ...
  } else if (change.from >= sel.from && change.to <= sel.to && change.to - change.from >= (sel.to - sel.from) / 3 && (!newSel || newSel.main.empty && newSel.main.from == change.from + change.insert.length) && view3.inputState.composing < 0) {
    let before = sel.from < change.from ? startState.sliceDoc(sel.from, change.from) : "";
    let after = sel.to > change.to ? startState.sliceDoc(change.to, sel.to) : "";
    tr = startState.replaceSelection(view3.state.toText(before + change.insert.sliceString(0, void 0, view3.state.lineBreak) + after));
  } else {
    let changes = startState.changes(change);
    let mainSel = newSel && newSel.main.to <= changes.newLength ? newSel.main : void 0;
    if (startState.selection.ranges.length > 1 && (view3.inputState.composing >= 0 || view3.inputState.compositionPendingChange) && change.to <= sel.to + 10 && change.to >= sel.to - 10) {
      let replaced = view3.state.sliceDoc(change.from, change.to);
      let compositionRange, composition = newSel && findCompositionNode(view3, newSel.main.head);
      if (composition) {
        let dLen = change.insert.length - (change.to - change.from);
        compositionRange = { from: composition.from, to: composition.to - dLen };
      } else {
        compositionRange = view3.state.doc.lineAt(sel.head);
      }
      ...
      tr = startState.changeByRange((range) => {
        ...
        if (view3.state.sliceDoc(from, to) != replaced || // Unfortunately, there's no way to make multiple
  ...
  if (view3.composing || view3.inputState.compositionPendingChange && view3.inputState.compositionEndedAt > Date.now() - 50) {
    view3.inputState.compositionPendingChange = false;
    userEvent += ".compose";
    if (view3.inputState.compositionFirstChange) {
      userEvent += ".start";
      view3.inputState.compositionFirstChange = false;
    }
  }
...
function selectionPoints(view3) {
  let result = [];
  if (view3.root.activeElement != view3.contentDOM)
    return result;
  let { anchorNode, anchorOffset, focusNode, focusOffset } = view3.observer.selectionRange;
  if (anchorNode) {
...
var InputState = class {
  ...
  constructor(view3) {
    ...
    this.notifiedFocused = view3.hasFocus;
    if (browser.safari)
      view3.contentDOM.addEventListener("input", () => null);
    if (browser.gecko)
      firefoxCopyCutHack(view3.contentDOM.ownerDocument);
  }
  handleEvent(event) {
...
function bindHandler(plugin, handler) {
  return (view3, event) => {
    try {
      return handler.call(plugin, event, view3);
    } catch (e) {
      logException(view3.state, e);
    }
  };
}
...
var MouseSelection = class {
  constructor(view3, startEvent, style, mustSelect) {
    this.view = view3;
    ...
    this.scrollParents = scrollableParents(view3.contentDOM);
    this.atoms = view3.state.facet(atomicRanges).map((f) => f(view3));
    let doc2 = view3.contentDOM.ownerDocument;
    doc2.addEventListener("mousemove", this.move = this.move.bind(this));
    doc2.addEventListener("mouseup", this.up = this.up.bind(this));
    this.extend = startEvent.shiftKey;
    this.multiple = view3.state.facet(EditorState.allowMultipleSelections) && addsSelectionRange(view3, startEvent);
    this.dragging = isInPrimarySelection(view3, startEvent) && getClickType(startEvent) == 1 ? null : false;
  }
  start(event) {
  ...
  select(event) {
    let { view: view3 } = this, selection = skipAtomsForSelection(this.atoms, this.style.get(event, this.extend, this.multiple));
    if (this.mustSelect || !selection.eq(view3.state.selection, this.dragging === false))
      this.view.dispatch({
        selection,
...
function addsSelectionRange(view3, event) {
  let facet = view3.state.facet(clickAddsSelectionRange);
  return facet.length ? facet[0](event) : browser.mac ? event.metaKey : event.ctrlKey;
}
function dragMovesSelection(view3, event) {
  let facet = view3.state.facet(dragMovesSelection$1);
  return facet.length ? facet[0](event) : browser.mac ? !event.altKey : !event.ctrlKey;
}
function isInPrimarySelection(view3, event) {
  let { main } = view3.state.selection;
  if (main.empty)
    return false;
  let sel = getSelection(view3.root);
...
function eventBelongsToEditor(view3, event) {
  if (!event.bubbles)
    return true;
  if (event.defaultPrevented)
    return false;
  for (let node = event.target, tile; node != view3.contentDOM; node = node.parentNode)
...
function capturePaste(view3) {
  let parent = view3.dom.parentNode;
  if (!parent)
    return;
...
function doPaste(view3, input) {
  input = textFilter(view3.state, clipboardInputFilter, input);
  let { state: state3 } = view3, changes, i2 = 1, text = state3.toText(input);
  ...
  view3.dispatch(changes, {
    userEvent: "input.paste",
    scrollIntoView: true
  });
}
observers.scroll = (view3) => {
  let iState = view3.inputState;
  iState.lastScrollTop = view3.scrollDOM.scrollTop;
  iState.lastScrollLeft = view3.scrollDOM.scrollLeft;
  if (browser.ios && !iState.touchActive)
    iState.lastIOSMomentumScroll = Date.now();
};
observers.wheel = observers.mousewheel = (view3) => {
  view3.inputState.lastWheelEvent = Date.now();
};
handlers.keydown = (view3, event) => {
  view3.inputState.setSelectionOrigin("select");
  if (event.keyCode == 27 && view3.inputState.tabFocusMode != 0)
    view3.inputState.tabFocusMode = Date.now() + 2e3;
  return false;
};
observers.touchstart = (view3, e) => {
  let iState = view3.inputState, touch = e.targetTouches[0];
...
observers.touchmove = (view3) => {
  view3.inputState.setSelectionOrigin("select.pointer");
};
observers.touchend = (view3, e) => {
  view3.inputState.touchActive = false;
};
handlers.mousedown = (view3, event) => {
  view3.observer.flush();
  if (view3.inputState.lastTouchTime > Date.now() - 2e3)
    return false;
  let style = null;
  for (let makeStyle of view3.state.facet(mouseSelectionStyle)) {
    style = makeStyle(view3, event);
    if (style)
      break;
  }
  if (!style && event.button == 0)
    style = basicMouseSelection(view3, event);
  if (style) {
    let mustFocus = !view3.hasFocus;
    view3.inputState.startMouseSelection(new MouseSelection(view3, event, style, mustFocus));
    if (mustFocus)
      view3.observer.ignore(() => {
        focusPreventScroll(view3.contentDOM);
        let active = view3.root.activeElement;
        if (active && !active.contains(view3.contentDOM))
          active.blur();
      });
    let mouseSel = view3.inputState.mouseSelection;
    if (mouseSel) {
...
function rangeForClick(view3, pos, bias, type) {
  if (type == 1) {
    return EditorSelection.cursor(pos, bias);
  } else if (type == 2) {
    return groupAt(view3.state, pos, bias);
  } else {
    let visual = view3.docView.lineAt(pos, bias), line = view3.state.doc.lineAt(visual ? visual.posAtEnd : pos);
    let from = visual ? visual.posAtStart : line.from, to = visual ? visual.posAtEnd : line.to;
    if (to < view3.state.doc.length && to == line.to)
      to++;
...
function basicMouseSelection(view3, event) {
  let start = view3.posAndSideAtCoords({ x: event.clientX, y: event.clientY }, false), type = getClickType(event);
  let startSel = view3.state.selection;
  return {
    update(update) {
    ...
    get(event2, extend, multiple) {
      let cur = view3.posAndSideAtCoords({ x: event2.clientX, y: event2.clientY }, false), removed;
      let range = rangeForClick(view3, cur.pos, cur.assoc, type);
      if (start.pos != cur.pos && !extend) {
        let startRange = rangeForClick(view3, start.pos, start.assoc, type);
...
handlers.dragstart = (view3, event) => {
  let { selection: { main: range } } = view3.state;
  if (event.target.draggable) {
    let tile = view3.docView.tile.nearest(event.target);
  ...
  let { inputState } = view3;
  if (inputState.mouseSelection)
    inputState.mouseSelection.dragging = true;
  inputState.draggedContent = range;
  if (event.dataTransfer) {
    event.dataTransfer.setData("Text", textFilter(view3.state, clipboardOutputFilter, view3.state.sliceDoc(range.from, range.to)));
    event.dataTransfer.effectAllowed = "copyMove";
  }
  return false;
};
handlers.dragend = (view3) => {
  view3.inputState.draggedContent = null;
  return false;
};
function dropText(view3, event, text, direct) {
  text = textFilter(view3.state, clipboardInputFilter, text);
  if (!text)
    return;
  let dropPos = view3.posAtCoords({ x: event.clientX, y: event.clientY }, false);
  let { draggedContent } = view3.inputState;
  let del = direct && draggedContent && dragMovesSelection(view3, event) ? { from: draggedContent.from, to: draggedContent.to } : null;
  let ins = { from: dropPos, insert: text };
  let changes = view3.state.changes(del ? [del, ins] : ins);
  view3.focus();
  view3.dispatch({
    changes,
  ...
  if (files && files.length) {
    ...
    let finishFile = () => {
      if (++read == files.length)
        dropText(view3, event, text.filter((s) => s != null).join(view3.state.lineBreak), false);
    };
    ...
    if (text) {
      dropText(view3, event, text, true);
      return true;
    }
  }
  return false;
};
handlers.paste = (view3, event) => {
  if (view3.state.readOnly)
    return true;
  view3.observer.flush();
  let data = brokenClipboardAPI ? null : event.clipboardData;
  if (data) {
    doPaste(view3, data.getData("text/plain") || data.getData("text/uri-list"));
    return true;
  } else {
    capturePaste(view3);
    return false;
...
function captureCopy(view3, text) {
  let parent = view3.dom.parentNode;
  if (!parent)
    return;
  let target = parent.appendChild(document.createElement("textarea"));
...
handlers.copy = handlers.cut = (view3, event) => {
  if (!hasSelection(view3.contentDOM, view3.observer.selectionRange))
    return false;
  let { text, ranges, linewise } = copiedRange(view3.state);
  if (!text && !linewise)
    return false;
  lastLinewiseCopy = linewise ? text : null;
  if (event.type == "cut" && !view3.state.readOnly)
    view3.dispatch({
      changes: ranges,
      scrollIntoView: true,
...
function updateForFocusChange(view3) {
  setTimeout(() => {
    let focus = view3.hasFocus;
    if (focus != view3.inputState.notifiedFocused) {
      let tr = focusChangeTransaction(view3.state, focus);
      if (tr)
        view3.dispatch(tr);
      else
        view3.update([]);
...
observers.focus = (view3) => {
  view3.inputState.lastFocusTime = Date.now();
  if (!view3.scrollDOM.scrollTop && (view3.inputState.lastScrollTop || view3.inputState.lastScrollLeft)) {
    view3.scrollDOM.scrollTop = view3.inputState.lastScrollTop;
    view3.scrollDOM.scrollLeft = view3.inputState.lastScrollLeft;
  }
  updateForFocusChange(view3);
};
observers.blur = (view3) => {
  view3.observer.clearSelectionRange();
  updateForFocusChange(view3);
};
observers.compositionstart = observers.compositionupdate = (view3) => {
  if (view3.observer.editContext)
    return;
  if (view3.inputState.compositionFirstChange == null)
    view3.inputState.compositionFirstChange = true;
  if (view3.inputState.composing < 0) {
    view3.inputState.composing = 0;
  }
};
observers.compositionend = (view3) => {
  if (view3.observer.editContext)
    return;
  view3.inputState.composing = -1;
  view3.inputState.compositionEndedAt = Date.now();
  view3.inputState.compositionPendingKey = true;
  view3.inputState.compositionPendingChange = view3.observer.pendingRecords().length > 0;
  view3.inputState.compositionFirstChange = null;
  if (browser.chrome && browser.android) {
    view3.observer.flushSoon();
  } else if (view3.inputState.compositionPendingChange) {
    Promise.resolve().then(() => view3.observer.flush());
  } else {
    setTimeout(() => {
      if (view3.inputState.composing < 0 && view3.docView.hasComposition)
        view3.update([]);
    }, 50);
  }
};
observers.contextmenu = (view3) => {
  view3.inputState.lastContextMenu = Date.now();
};
handlers.beforeinput = (view3, event) => {
  var _a2, _b;
  if (event.inputType == "insertText" || event.inputType == "insertCompositionText") {
    view3.inputState.insertingText = event.data;
    view3.inputState.insertingTextAt = Date.now();
  }
  if (event.inputType == "insertReplacementText" && view3.observer.editContext) {
    let text = (_a2 = event.dataTransfer) === null || _a2 === void 0 ? void 0 : _a2.getData("text/plain"), ranges = event.getTargetRanges();
    if (text && ranges.length) {
      let r = ranges[0];
      let from = view3.posAtDOM(r.startContainer, r.startOffset), to = view3.posAtDOM(r.endContainer, r.endOffset);
      applyDOMChangeInner(view3, { from, to, insert: view3.state.toText(text) }, null);
      return true;
    }
  }
  let pending;
  if (browser.chrome && browser.android && (pending = PendingKeys.find((key) => key.inputType == event.inputType))) {
    view3.observer.delayAndroidKey(pending.key, pending.keyCode);
    if (pending.key == "Backspace" || pending.key == "Delete") {
      let startViewHeight = ((_b = window.visualViewport) === null || _b === void 0 ? void 0 : _b.height) || 0;
      setTimeout(() => {
        var _a3;
        if ((((_a3 = window.visualViewport) === null || _a3 === void 0 ? void 0 : _a3.height) || 0) > startViewHeight + 10 && view3.hasFocus) {
          view3.contentDOM.blur();
          view3.focus();
        }
      }, 100);
    }
  ...
  if (browser.safari && event.inputType == "insertText" && view3.inputState.composing >= 0) {
    setTimeout(() => observers.compositionend(view3, event), 20);
  }
  return false;
};
...
var ViewState = class {
  constructor(view3, state3) {
    ...
    this.stateDeco = staticDeco(state3);
    ...
    this.scrollParent = view3.scrollDOM;
  ...
  measure() {
    let { view: view3 } = this, dom = view3.contentDOM, style = window.getComputedStyle(dom);
    ...
    if (this.editorWidth != view3.scrollDOM.clientWidth) {
      if (oracle.lineWrapping)
        measureContent = true;
      this.editorWidth = view3.scrollDOM.clientWidth;
      result |= 16;
    }
    ...
    this.scrolledToBottom = isScrolledToBottom(this.scrollParent || view3.win);
    ...
    if (!this.inView && !this.scrollTarget && !inWindow(view3.dom))
      return 0;
    let contentWidth = domRect.width;
    if (this.contentDOMWidth != contentWidth || this.editorHeight != view3.scrollDOM.clientHeight) {
      this.contentDOMWidth = domRect.width;
      this.editorHeight = view3.scrollDOM.clientHeight;
      result |= 16;
    }
    if (measureContent) {
      let lineHeights = view3.docView.measureVisibleLineHeights(this.viewport);
      if (oracle.mustRefreshForHeights(lineHeights))
        refresh = true;
      if (refresh || oracle.lineWrapping && Math.abs(contentWidth - this.contentDOMWidth) > oracle.charWidth) {
        let { lineHeight, charWidth, textHeight } = view3.docView.measureTextSize();
        refresh = lineHeight > 0 && oracle.refresh(whiteSpace, lineHeight, charWidth, textHeight, Math.max(5, contentWidth / charWidth), lineHeights);
        if (refresh) {
          view3.docView.minWidth = 0;
          result |= 16;
        }
      }
      if (dTop > 0 && dBottom > 0)
      ...
      for (let vp of this.viewports) {
        let heights = vp.from == this.viewport.from ? lineHeights : view3.docView.measureVisibleLineHeights(vp);
        this.heightMap = (refresh ? HeightMap.empty().applyChanges(this.stateDeco, Text.empty, this.heightOracle, [new ChangedRange(0, 0, 0, view3.state.doc.length)]) : this.heightMap).updateHeight(oracle, 0, refresh, new MeasuredHeights(vp.from, heights));
      }
      if (heightChangeFlag)
        result |= 2;
    }
    ...
    if (result & 2 || viewportChange)
      this.updateViewportLines();
    if (this.lineGaps.length || this.viewport.to - this.viewport.from > 2e3 << 1)
      this.updateLineGaps(this.ensureLineGaps(refresh ? [] : this.lineGaps, view3));
    result |= this.computeVisibleRanges();
    if (this.mustEnforceCursorAssoc) {
      this.mustEnforceCursorAssoc = false;
      view3.docView.enforceCursorAssoc();
    }
    return result;
  }
  get visibleTop() {
...
function staticDeco(state3) {
  let deco = state3.facet(decorations).filter((d) => typeof d != "function");
  let outer = state3.facet(outerDecorations).filter((d) => typeof d != "function");
  if (outer.length)
...
var DOMObserver = class {
  constructor(view3) {
    ...
    this.parentCheck = -1;
    this.dom = view3.contentDOM;
    this.observer = new MutationObserver((mutations) => {
      for (let mut of mutations)
        this.queue.push(mut);
      if ((browser.ie && browser.ie_version <= 11 || browser.ios && view3.composing) && mutations.some((m) => m.type == "childList" && m.removedNodes.length || m.type == "characterData" && m.oldValue.length > m.target.nodeValue.length))
        this.flushSoon();
      else
        this.flush();
    });
    if (window.EditContext && browser.android && view3.constructor.EDIT_CONTEXT !== false && // Chrome <126 doesn't support inverted selections in edit context (#1392)
    !(browser.chrome && browser.chrome_version < 126)) {
      this.editContext = new EditContextManager(view3);
      if (view3.state.facet(editable))
        view3.contentDOM.editContext = this.editContext.editContext;
    }
    if (useCharData)
    ...
    if (typeof ResizeObserver == "function") {
      ...
      this.resizeScroll.observe(view3.scrollDOM);
    }
    this.addWindowListeners(this.win = view3.win);
    this.start();
  ...
  onSelectionChange(event) {
    ...
    if (!this.readSelectionRange() || this.delayedAndroidKey)
      return;
    let { view: view3 } = this, sel = this.selectionRange;
    if (view3.state.facet(editable) ? view3.root.activeElement != this.dom : !hasSelection(this.dom, sel))
      return;
    let context = sel.anchorNode && view3.docView.tile.nearest(sel.anchorNode);
    if (context && context.isWidget() && context.widget.ignoreEvent(event)) {
      if (!wasChanged)
        this.selectionChanged = false;
      return;
    }
    if ((browser.ie && browser.ie_version <= 11 || browser.android && browser.chrome) && !view3.state.selection.main.empty && // (Selection.isCollapsed isn't reliable on IE)
  ...
  readSelectionRange() {
    let { view: view3 } = this;
    let selection = getSelection(view3.root);
    if (!selection)
      return false;
    let range = browser.safari && view3.root.nodeType == 11 && view3.root.activeElement == this.dom && safariSelectionRangeHack(this.view, selection) || selection;
    if (!range || this.selectionRange.eq(range))
      return false;
    let local = hasSelection(this.dom, range);
    if (local && !this.selectionChanged && view3.inputState.lastFocusTime > Date.now() - 200 && view3.inputState.lastTouchTime < Date.now() - 300 && atElementStart(this.dom, range)) {
      this.view.inputState.lastFocusTime = 0;
      view3.docView.updateSelection();
      return false;
    }
...
function buildSelectionRangeFromRange(view3, range) {
  let anchorNode = range.startContainer, anchorOffset = range.startOffset;
  let focusNode = range.endContainer, focusOffset = range.endOffset;
  let curAnchor = view3.docView.domAtPos(view3.state.selection.main.anchor, 1);
  if (isEquivalentPosition(curAnchor.node, curAnchor.offset, focusNode, focusOffset))
    [anchorNode, anchorOffset, focusNode, focusOffset] = [focusNode, focusOffset, anchorNode, anchorOffset];
  return { anchorNode, anchorOffset, focusNode, focusOffset };
}
function safariSelectionRangeHack(view3, selection) {
  if (selection.getComposedRanges) {
    let range = selection.getComposedRanges(view3.root)[0];
    if (range)
      return buildSelectionRangeFromRange(view3, range);
  }
  let found = null;
  function read(event) {
    event.preventDefault();
  ...
  view3.contentDOM.addEventListener("beforeinput", read, true);
  view3.dom.ownerDocument.execCommand("indent");
  view3.contentDOM.removeEventListener("beforeinput", read, true);
  return found ? buildSelectionRangeFromRange(view3, found) : null;
}
var EditContextManager = class {
  constructor(view3) {
    ...
    this.handlers = /* @__PURE__ */ Object.create(null);
    this.composing = null;
    this.resetRange(view3.state);
    let context = this.editContext = new window.EditContext({
      text: view3.state.doc.sliceString(this.from, this.to),
      selectionStart: this.toContextPos(Math.max(this.from, Math.min(this.to, view3.state.selection.main.anchor))),
      selectionEnd: this.toContextPos(view3.state.selection.main.head)
    });
    this.handlers.textupdate = (e) => {
      let main = view3.state.selection.main, { anchor, head } = main;
      let from = this.toEditorPos(e.updateRangeStart), to = this.toEditorPos(e.updateRangeEnd);
      if (view3.inputState.composing >= 0 && !this.composing)
      ...
      if (from == this.from && anchor < this.from)
        ...
        to = anchor;
      let diff = findDiff(view3.state.sliceDoc(from, to), e.text, (deletes ? main.from : main.to) - from, deletes ? "end" : null);
      if (!diff) {
        let newSel = EditorSelection.single(this.toEditorPos(e.selectionStart), this.toEditorPos(e.selectionEnd));
        if (!sameSelPos(newSel, main))
          view3.dispatch({ selection: newSel, userEvent: "select" });
        return;
      }
      let change = {
        from: diff.from + from,
      ...
      if ((browser.mac || browser.android) && change.from == head - 1 && /^\. ?$/.test(e.text) && view3.contentDOM.getAttribute("autocorrect") == "off")
        change = { from, to, insert: Text.of([e.text.replace(".", " ")]) };
      this.pendingContextChange = change;
      if (!view3.state.readOnly) {
        let newLen = this.to - this.from + (change.to - change.from + change.insert.length);
        applyDOMChangeInner(view3, change, EditorSelection.single(this.toEditorPos(e.selectionStart, newLen), this.toEditorPos(e.selectionEnd, newLen)));
      }
      if (this.pendingContextChange) {
        this.revertPending(view3.state);
        this.setSelection(view3.state);
      }
      if (change.from < change.to && !change.insert.length && view3.inputState.composing >= 0 && !/[\\p{Alphabetic}\\p{Number}_]/.test(context.text.slice(Math.max(0, e.updateRangeStart - 1), Math.min(context.text.length, e.updateRangeStart + 1))))
        this.handlers.compositionend(e);
    };
    this.handlers.characterboundsupdate = (e) => {
      let rects = [], prev = null;
      for (let i2 = this.toEditorPos(e.rangeStart), end = this.toEditorPos(e.rangeEnd); i2 < end; i2++) {
        let rect = view3.coordsForChar(i2);
    ...
    this.handlers.textformatupdate = (e) => {
      ...
      view3.dispatch({ effects: setEditContextFormatting.of(Decoration.set(deco)) });
    };
    ...
    this.handlers.compositionend = () => {
      ...
      if (this.composing) {
        ...
        this.composing = null;
        if (drifted)
          this.reset(view3.state);
      }
    };
    for (let event in this.handlers)
      context.addEventListener(event, this.handlers[event]);
    this.measureReq = { read: (view4) => {
      let sel = getSelection(view4.root);
...
function attrsFromFacet(view3, facet, base2) {
  for (let sources = view3.state.facet(facet), i2 = sources.length - 1; i2 >= 0; i2--) {
    let source = sources[i2], value = typeof source == "function" ? source(view3) : source;
    if (value)
...
var handleKeyEvents = /* @__PURE__ */ Prec.default(/* @__PURE__ */ EditorView.domEventHandlers({
  keydown(event, view3) {
    return runHandlers(getKeymap(view3.state), event, view3, "editor");
  }
}));
    ...
    for (let i2 = 1; i2 < parts.length; i2++) {
      ...
      if (!scopeObj[prefix])
        scopeObj[prefix] = {
          ...
          run: [(view3) => {
            let ourObj = storedPrefix = { view: view3, prefix, scope };
            setTimeout(() => {
    ...
    if (b.any)
      for (let scope of scopes) {
        ...
        for (let key in scopeObj)
          scopeObj[key].run.push((view3) => any(view3, currentKeyEvent));
      }
...
function runHandlers(map, event, view3, scope) {
  currentKeyEvent = event;
  let name2 = keyName(event);
  let charCode = codePointAt2(name2, 0), isChar = codePointSize2(charCode) == name2.length && name2 != " ";
  let prefix = "", handled = false, prevented = false, stopPropagation = false;
  if (storedPrefix && storedPrefix.view == view3 && storedPrefix.scope == scope) {
...
var RectangleMarker = class _RectangleMarker {
  ...
  */
  static forRange(view3, className, range) {
    if (range.empty) {
      let pos = view3.coordsAtPos(range.head, range.assoc || 1);
      if (!pos)
        return [];
      let base2 = getBase(view3);
      return [new _RectangleMarker(className, pos.left - base2.left, pos.top - base2.top, null, pos.bottom - pos.top)];
    } else {
      return rectanglesForRange(view3, className, range);
    }
  }
};
function getBase(view3) {
  let rect = view3.scrollDOM.getBoundingClientRect();
  let left = view3.textDirection == Direction.LTR ? rect.left : rect.right - view3.scrollDOM.clientWidth * view3.scaleX;
  return { left: left - view3.scrollDOM.scrollLeft * view3.scaleX, top: rect.top - view3.scrollDOM.scrollTop * view3.scaleY };
}
function wrappedLine(view3, pos, side, inside) {
  let coords = view3.coordsAtPos(pos, side * 2);
  if (!coords)
    return inside;
  let editorRect = view3.dom.getBoundingClientRect();
  let y = (coords.top + coords.bottom) / 2;
  let left = view3.posAtCoords({ x: editorRect.left + 1, y });
  let right = view3.posAtCoords({ x: editorRect.right - 1, y });
  if (left == null || right == null)
    return inside;
  return { from: Math.max(inside.from, Math.min(left, right)), to: Math.min(inside.to, Math.max(left, right)) };
}
function rectanglesForRange(view3, className, range) {
  if (range.to <= view3.viewport.from || range.from >= view3.viewport.to)
    return [];
  let from = Math.max(range.from, view3.viewport.from), to = Math.min(range.to, view3.viewport.to);
  let ltr = view3.textDirection == Direction.LTR;
  let content2 = view3.contentDOM, contentRect = content2.getBoundingClientRect(), base2 = getBase(view3);
  let lineElt = content2.querySelector(".cm-line"), lineStyle = lineElt && window.getComputedStyle(lineElt);
  let leftSide = contentRect.left + (lineStyle ? parseInt(lineStyle.paddingLeft) + Math.min(0, parseInt(lineStyle.textIndent)) : 0);
  let rightSide = contentRect.right - (lineStyle ? parseInt(lineStyle.paddingRight) : 0);
  let startBlock = blockAt(view3, from, 1), endBlock = blockAt(view3, to, -1);
  let visualStart = startBlock.type == BlockType.Text ? startBlock : null;
  let visualEnd = endBlock.type == BlockType.Text ? endBlock : null;
  if (visualStart && (view3.lineWrapping || startBlock.widgetLineBreaks))
    visualStart = wrappedLine(view3, from, 1, visualStart);
  if (visualEnd && (view3.lineWrapping || endBlock.widgetLineBreaks))
    visualEnd = wrappedLine(view3, to, -1, visualEnd);
  if (visualStart && visualEnd && visualStart.from == visualEnd.from && visualStart.to == visualEnd.to) {
    ...
    let between = [];
    if ((visualStart || startBlock).to < (visualEnd || endBlock).from - (visualStart && visualEnd ? 1 : 0) || startBlock.widgetLineBreaks > 1 && top2.bottom + view3.defaultLineHeight / 2 < bottom.top)
      between.push(piece(leftSide, top2.bottom, rightSide, bottom.top));
    else if (top2.bottom < bottom.top && view3.elementAtHeight((top2.bottom + bottom.top) / 2).type == BlockType.Text)
  ...
  function drawForLine(from2, to2, line) {
    ...
    function addSpan(from3, fromOpen, to3, toOpen, dir) {
      let fromCoords = view3.coordsAtPos(from3, from3 == line.to ? -2 : 2);
      let toCoords = view3.coordsAtPos(to3, to3 == line.from ? 2 : -2);
    ...
    for (let r of view3.visibleRanges)
      if (r.to > start && r.from < end) {
        for (let pos = Math.max(r.from, start), endPos = Math.min(r.to, end); ; ) {
          let docLine = view3.state.doc.lineAt(pos);
          for (let span of view3.bidiSpans(docLine)) {
    ...
    if (horizontal.length == 0)
      addSpan(start, from2 == null, end, to2 == null, view3.textDirection);
...
var LayerView = class {
  constructor(view3, layer2) {
    ...
    this.dom = view3.scrollDOM.appendChild(document.createElement("div"));
    ...
    this.setOrder(view3.state);
    view3.requestMeasure(this.measureReq);
    if (layer2.mount)
      layer2.mount(this.dom, view3);
  }
  update(update) {
...
var cursorLayer = /* @__PURE__ */ layer({
  above: true,
  markers(view3) {
    let { state: state3 } = view3, conf = state3.facet(selectionConfig);
    let cursors = [];
    for (let r of state3.selection.ranges) {
      ...
      if (r.empty || conf.drawRangeCursor && !(prim && browser.ios && conf.iosSelectionHandles)) {
        ...
        for (let piece of RectangleMarker.forRange(view3, className, cursor))
          cursors.push(piece);
      }
    }
    return cursors;
...
var selectionLayer = /* @__PURE__ */ layer({
  above: false,
  markers(view3) {
    let markers = [], { main, ranges } = view3.state.selection;
    for (let r of ranges)
      if (!r.empty) {
        for (let marker of RectangleMarker.forRange(view3, "cm-selectionBackground", r))
          markers.push(marker);
      }
    if (browser.ios && !main.empty && view3.state.facet(selectionConfig).iosSelectionHandles) {
      for (let piece of RectangleMarker.forRange(view3, "cm-selectionHandle cm-selectionHandle-start", EditorSelection.cursor(main.from, 1)))
        markers.push(piece);
      for (let piece of RectangleMarker.forRange(view3, "cm-selectionHandle cm-selectionHandle-end", EditorSelection.cursor(main.to, 1)))
        markers.push(piece);
    }
    return markers;
  },
...
var activeLineHighlighter = /* @__PURE__ */ ViewPlugin.fromClass(class {
  constructor(view3) {
    this.decorations = this.getDeco(view3);
  }
  update(update) {
  ...
  getDeco(view3) {
    ...
    for (let r of view3.state.selection.ranges) {
      let line = view3.lineBlockAt(r.head);
...
var gutterView = /* @__PURE__ */ ViewPlugin.fromClass(class {
  constructor(view3) {
    this.view = view3;
    this.domAfter = null;
    this.prevViewport = view3.viewport;
    this.dom = document.createElement("div");
    this.dom.className = "cm-gutters cm-gutters-before";
    this.dom.setAttribute("aria-hidden", "true");
    this.dom.style.minHeight = this.view.contentHeight / this.view.scaleY + "px";
    this.gutters = view3.state.facet(activeGutters).map((conf) => new SingleGutterView(view3, conf));
    this.fixed = !view3.state.facet(unfixGutters);
    ...
    view3.scrollDOM.insertBefore(this.dom, view3.contentDOM);
  }
  getDOMAfter() {
    if (!this.domAfter) {
  ...
  provide: (plugin) => EditorView.scrollMargins.of((view3) => {
    let value = view3.plugin(plugin);
    if (!value || value.gutters.length == 0 || !value.fixed)
      return null;
    let before = value.dom.offsetWidth * view3.scaleX, after = value.domAfter ? value.domAfter.offsetWidth * view3.scaleX : 0;
    return view3.textDirection == Direction.LTR ? { left: before, right: after } : { right: before, left: after };
  })
});
...
var UpdateContext = class {
  ...
  addElement(view3, block, markers) {
    let { gutter: gutter2 } = this, above = (block.top - this.height) / view3.scaleY, height = block.height / view3.scaleY;
    if (this.i == gutter2.elements.length) {
      let newElt = new GutterElement(view3, height, above, markers);
      gutter2.elements.push(newElt);
      gutter2.dom.appendChild(newElt.dom);
    } else {
      gutter2.elements[this.i].update(view3, height, above, markers);
    }
    this.height = block.bottom;
    this.i++;
  }
  line(view3, line, extraMarkers) {
    let localMarkers = [];
    advanceCursor(this.cursor, localMarkers, line.from);
    if (extraMarkers.length)
      localMarkers = localMarkers.concat(extraMarkers);
    let forLine = this.gutter.config.lineMarker(view3, line, localMarkers);
    if (forLine)
      localMarkers.unshift(forLine);
    let gutter2 = this.gutter;
    if (localMarkers.length == 0 && !gutter2.config.renderEmptyElements)
      return;
    this.addElement(view3, line, localMarkers);
  }
  widget(view3, block) {
    let marker = this.gutter.config.widgetMarker(view3, block.widget, block), markers = marker ? [marker] : null;
    for (let cls of view3.state.facet(gutterWidgetClass)) {
      let marker2 = cls(view3, block.widget, block);
      if (marker2)
        (markers || (markers = [])).push(marker2);
    }
    if (markers)
      this.addElement(view3, block, markers);
  }
  finish() {
    let gutter2 = this.gutter;
  ...
  constructor(view3, config) {
    ...
    for (let prop in config.domEventHandlers) {
      this.dom.addEventListener(prop, (event) => {
        ...
        let line = view3.lineBlockAtHeight(y - view3.documentTop);
        if (config.domEventHandlers[prop](view3, line, event))
          event.preventDefault();
      });
    }
    this.markers = asArray2(config.markers(view3));
    if (config.initialSpacer) {
      this.spacer = new GutterElement(view3, 0, 0, [config.initialSpacer(view3)]);
...
var GutterElement = class {
  constructor(view3, height, above, markers) {
    this.height = -1;
    this.above = 0;
    this.markers = [];
    this.dom = document.createElement("div");
    this.dom.className = "cm-gutterElement";
    this.update(view3, height, above, markers);
  }
  update(view3, height, above, markers) {
    if (this.height != height) {
      this.height = height;
    ...
    if (!sameMarkers(this.markers, markers))
      this.setMarkers(view3, markers);
  }
  setMarkers(view3, markers) {
    ...
    for (let iNew = 0, iOld = 0; ; ) {
      ...
      if (marker.toDOM) {
        if (matched)
          ...
          this.dom.insertBefore(marker.toDOM(view3), domPos);
      }
      if (matched)
        iOld++;
    }
    ...
    return combineConfig(values, { formatNumber: String, domEventHandlers: {} }, {
      domEventHandlers(a, b) {
        ...
        for (let event in b) {
          ...
          result[event] = exists ? (view3, line, event2) => exists(view3, line, event2) || add(view3, line, event2) : add;
        }
        return result;
      }
    });
...
function formatNumber(view3, number2) {
  return view3.state.facet(lineNumberConfig).formatNumber(number2, view3.state);
}
var lineNumberGutter = /* @__PURE__ */ activeGutters.compute([lineNumberConfig], (state3) => ({
  class: "cm-lineNumbers",
  renderEmptyElements: false,
  markers(view3) {
    return view3.state.facet(lineNumberMarkers);
  },
  lineMarker(view3, line, others) {
    if (others.some((m) => m.toDOM))
      return null;
    return new NumberMarker(formatNumber(view3, view3.state.doc.lineAt(line.from).number));
  },
  widgetMarker: (view3, widget, block) => {
    for (let m of view3.state.facet(lineNumberWidgetMarker)) {
      let result = m(view3, widget, block);
      if (result)
        return result;
    }
    return null;
  ...
  initialSpacer(view3) {
    return new NumberMarker(formatNumber(view3, maxLineNumber(view3.state.doc.lines)));
  },
...
var ParseContext = class _ParseContext {
  constructor(parser2, state3, fragments = [], tree, treeLen, viewport, skipped, scheduleOn) {
    this.parser = parser2;
    this.state = state3;
  ...
  @internal
  */
  static create(parser2, state3, viewport) {
    return new _ParseContext(parser2, state3, [], Tree.empty, 0, viewport, [], null);
  }
  startParse() {
...
var TreeHighlighter = class {
  constructor(view3) {
    ...
    this.tree = syntaxTree(view3.state);
    this.decorations = this.buildDeco(view3, getHighlighters(view3.state));
    this.decoratedTo = view3.viewport.to;
  }
  update(update) {
  ...
  buildDeco(view3, highlighters) {
    if (!highlighters || !this.tree.length)
      return Decoration.none;
    let builder = new RangeSetBuilder();
    for (let { from, to } of view3.visibleRanges) {
...
function matchPlainBrackets(state3, pos, dir, tree, tokenType, maxScanDistance, brackets) {
  if (dir < 0 ? !pos : pos == state3.doc.length)
    return null;
...
function getConfig(state3, pos) {
  let data = state3.languageDataAt("commentTokens", pos, 1);
...
function changeLineComment(option, state3, ranges = state3.selection.ranges) {
  ...
  ranges: for (let { from, to } of ranges) {
    ...
    for (let pos = from; pos <= to; ) {
      ...
      if (token == void 0) {
        token = getConfig(state3, line.from).line;
        if (!token)
          continue ranges;
      }
...
var HistoryState = class _HistoryState {
  ...
  pop(side, state3, onlySelection) {
    ...
    if (branch.length == 0)
      return null;
    let event = branch[branch.length - 1], selection = event.selectionsAfter[0] || (event.startSelection ? event.startSelection.map(event.changes.invertedDesc, 1) : state3.selection);
...
function cursorByGroup(view3, forward) {
...
function cursorByLine(view3, forward) {
...
function cursorByPage(view3, forward) {
  let page = pageInfo(view3);
  let { state: state3 } = view3, selection = updateSel(state3.selection, (range) => {
...
function moveByLineBoundary(view3, start, forward) {
  ...
  if (!forward && moved.head == line.from && line.length) {
    let space2 = /^\s*/.exec(view3.state.sliceDoc(line.from, Math.min(line.from + 100, line.to)))[0].length;
...
function selectByChar(view3, forward) {
  return extendSel(view3, forward, (range) => view3.moveByChar(range, forward));
}
...
function selectByGroup(view3, forward) {
  return extendSel(view3, forward, (range) => view3.moveByGroup(range, forward));
}
...
var selectSyntaxLeft = (view3) => {
  let forward = !ltrAtCursor(view3);
  return extendSel(view3, forward, (range) => moveBySyntax(view3.state, range, forward));
};
var selectSyntaxRight = (view3) => {
  let forward = ltrAtCursor(view3);
  return extendSel(view3, forward, (range) => moveBySyntax(view3.state, range, forward));
};
function selectByLine(view3, forward) {
  return extendSel(view3, forward, (range) => view3.moveVertically(range, forward));
}
var selectLineUp = (view3) => selectByLine(view3, false);
var selectLineDown = (view3) => selectByLine(view3, true);
function selectByPage(view3, forward) {
  return extendSel(view3, forward, (range) => view3.moveVertically(range, forward, pageInfo(view3).height));
}
var selectPageUp = (view3) => selectByPage(view3, false);
var selectPageDown = (view3) => selectByPage(view3, true);
var selectLineBoundaryForward = (view3) => extendSel(view3, true, (range) => moveByLineBoundary(view3, range, true));
var selectLineBoundaryBackward = (view3) => extendSel(view3, false, (range) => moveByLineBoundary(view3, range, false));
var selectLineBoundaryLeft = (view3) => {
  let forward = !ltrAtCursor(view3);
  return extendSel(view3, forward, (range) => moveByLineBoundary(view3, range, forward));
};
var selectLineBoundaryRight = (view3) => {
  let forward = ltrAtCursor(view3);
  return extendSel(view3, forward, (range) => moveByLineBoundary(view3, range, forward));
};
var selectLineStart = (view3) => extendSel(view3, false, (range) => EditorSelection.cursor(view3.lineBlockAt(range.head).from));
var selectLineEnd = (view3) => extendSel(view3, true, (range) => EditorSelection.cursor(view3.lineBlockAt(range.head).to));
...
function addCursorVertically(view3, forward) {
  let { state: state3 } = view3, sel = state3.selection, ranges = state3.selection.ranges.slice();
  for (let range of state3.selection.ranges) {
    let line = state3.doc.lineAt(range.head);
    if (forward ? line.to < view3.state.doc.length : line.from > 0)
      for (let cur = range; ; ) {
        let next = view3.moveVertically(cur, forward);
  ...
  if (ranges.length == sel.ranges.length)
    return false;
  view3.dispatch(setSel(state3, EditorSelection.create(ranges, ranges.length - 1)));
  return true;
}
var addCursorAbove = (view3) => addCursorVertically(view3, false);
var addCursorBelow = (view3) => addCursorVertically(view3, true);
...
var deleteCharBackward = (view3) => deleteByChar(view3, false, true);
var deleteCharForward = (view3) => deleteByChar(view3, true, false);
...
var deleteToLineEnd = (view3) => deleteBy(view3, (range) => {
  let lineEnd = view3.lineBlockAt(range.head).to;
  return range.head < lineEnd ? lineEnd : Math.min(view3.state.doc.length, range.head + 1);
});
var deleteLineBoundaryBackward = (view3) => deleteBy(view3, (range) => {
  let lineStart = view3.moveToLineBoundary(range, false).head;
  return range.head > lineStart ? lineStart : Math.max(0, range.head - 1);
});
var deleteLineBoundaryForward = (view3) => deleteBy(view3, (range) => {
  let lineStart = view3.moveToLineBoundary(range, true).head;
  return range.head < lineStart ? lineStart : Math.min(view3.state.doc.length, range.head + 1);
});
...
function copyLine(state3, dispatch, forward) {
  ...
  dispatch(state3.update({
    changes: changeSet,
    selection: state3.selection.map(changeSet, forward ? 1 : -1),
    scrollIntoView: true,
...
var deleteLine = (view3) => {
  if (view3.state.readOnly)
    return false;
  let { state: state3 } = view3, changes = state3.changes(selectedLineBlocks(state3).map(({ from, to }) => {
    if (from > 0)
      from--;
  ...
  let selection = updateSel(state3.selection, (range) => {
    let dist2 = void 0;
    if (view3.lineWrapping) {
      let block = view3.lineBlockAt(range.head), pos = view3.coordsAtPos(range.head, range.assoc || 1);
      if (pos)
        dist2 = block.bottom + view3.documentTop - pos.bottom + view3.defaultLineHeight / 2;
    }
    return view3.moveVertically(range, true, dist2);
  }).map(changes);
...
function newlineAndIndent(atEof) {
  return ({ state: state3, dispatch }) => {
    ...
    let changes = state3.changeByRange((range) => {
      ...
      if (indent2 == null)
        indent2 = countColumn(/^\s*/.exec(state3.doc.lineAt(from).text)[0], state3.tabSize);
      ...
      if (explode)
        ...
        from = line.from;
      let insert2 = ["", indentString(state3, indent2)];
      if (explode)
...
var indentSelection = ({ state: state3, dispatch }) => {
  ...
  let changes = changeBySelectedLine(state3, (line, changes2, range) => {
    ...
    let norm = indentString(state3, indent2);
...
var indentLess = ({ state: state3, dispatch }) => {
  ...
  dispatch(state3.update(changeBySelectedLine(state3, (line, changes) => {
    ...
    if (!space2)
      return;
    let col = countColumn(space2, state3.tabSize), keep = 0;
...
var toggleTabFocusMode = (view3) => {
  view3.setTabFocusMode();
  return true;
};
  ...
  findForcedReduction() {
    ...
    let explore = (state3, depth) => {
      if (seen.includes(state3))
        return;
      seen.push(state3);
      return parser2.allActions(state3, (action) => {
...
function readToken(data, input, stack, group, precTable, precOffset) {
  ...
  scan: for (; ; ) {
    ...
    for (let i2 = state3 + 3; i2 < accEnd; i2 += 2)
...
var TokenCache = class {
  ...
  addActions(stack, token, end, index) {
    let { state: state3 } = stack, { parser: parser2 } = stack.p, { data } = parser2;
    ...
    for (let line of this.lines) {
      if (text.length) {
        ...
        for (let i2 = 0; i2 < tabs; i2++)
          indent2 += state3.facet(indentUnit);
...
var snippetPointerHandler = /* @__PURE__ */ EditorView.domEventHandlers({
  mousedown(event, view3) {
    let active = view3.state.field(snippetState, false), pos;
    if (!active || (pos = view3.posAtCoords({ x: event.clientX, y: event.clientY })) == null)
      return false;
  ...
  parser: /* @__PURE__ */ parser.configure({
    props: [
      ...
      /* @__PURE__ */ foldNodeProp.add({
        ...
        Body: (node, state3) => ({ from: node.from + 1, to: node.to - (node.to == state3.doc.length ? 0 : 1) }),
        "String FormatString": (node, state3) => ({ from: state3.doc.lineAt(node.from).to, to: node.to })
      })
    ]
  }),
  languageData: {
...
var limitCodeLength = EditorView.updateListener.of((update) => {
  ...
  if (code2.length > MAX_CODE_LENGTH) {
...
var roomCode = String(window.location.href.split("#")[1]);
